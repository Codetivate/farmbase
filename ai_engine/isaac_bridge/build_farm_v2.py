...
def main():
    ...
    rack_x_left  = -(AISLE/2 + RACK_D/2)
    rack_x_right =  (AISLE/2 + RACK_D/2)

    for side_idx, (rx, face_dir) in enumerate([
        (rack_x_left, 1),    # Left rack — outer edge faces aisle (right)
        (rack_x_right, -1),  # Right rack — outer edge faces aisle (left)
    ]):
        ...
        for t in range(TIERS):
            ...
            # ── NFT Gutter (U-channel, nearly flat, 5° slope for flow) ──
            # Gutter sits ON the shelf, extends toward aisle
            gut_cx = rx + face_dir * GUTTER_W * 0.5
            gut_cz = tz + 0.02  # sits on shelf

            # Gutter base
            VisualCuboid(f"/World/Racks/{side}/Gut_T{t}", name=f"gut_{side}_t{t}",
                         position=np.array([gut_cx, 0, gut_cz]),
                         scale=np.array([GUTTER_W, RACK_L, 0.008]),
                         color=WHITE_PANEL * 0.95)
            # Gutter back wall (inner side, against rack)
            VisualCuboid(f"/World/Racks/{side}/GutBack_T{t}", name=f"gutback_{side}_t{t}",
                         position=np.array([gut_cx - face_dir*(GUTTER_W/2-0.003), 0, gut_cz + GUTTER_D/2]),
                         scale=np.array([0.006, RACK_L, GUTTER_D]),
                         color=WHITE_PANEL * 0.92)
            # Gutter front lip (outer edge — where crowns sit, lower lip for fruit to hang)
            VisualCuboid(f"/World/Racks/{side}/GutFront_T{t}", name=f"gutfront_{side}_t{t}",
                         position=np.array([gut_cx + face_dir*(GUTTER_W/2-0.003), 0, gut_cz + GUTTER_D*0.3]),
                         scale=np.array([0.006, RACK_L, GUTTER_D * 0.6]),
                         color=WHITE_PANEL * 0.92)
            # ↑ Front lip is SHORTER than back wall — fruit can hang over!
            ...
            for pi in range(pc):
                ...
                # Crown/leaf cluster — positioned at the OUTER EDGE of gutter
                crown_x = gut_cx + face_dir * (GUTTER_W/2 - 0.04)
                crown_z = gut_cz + 0.06
                ...
                # Fruit HANGING BELOW gutter — pendant habit (gravity)
                berry_x = crown_x + face_dir * 0.03  # slightly further out
                berry_z = gut_cz - 0.06  # BELOW the gutter base!
    ...
    for si, rx in enumerate([rack_x_left, rack_x_right]):
        sn = "L" if si == 0 else "R"
        VisualCuboid(f"/World/Racks/{sn}/Tray", name=f"tray_{sn}",
    ...
    # Vertical manifold pipe per rack side
    for si, rx in enumerate([rack_x_left, rack_x_right]):
        sn = "L" if si == 0 else "R"
        fd = 1 if si == 0 else -1
        header_x = rx + fd * GUTTER_W * 0.5 + fd * 0.02
        header_z_top = T_BASE + (TIERS-1) * T_GAP + 0.05
        # Vertical header pipe
        VisualCuboid(f"/World/Irrig/Header_{sn}", name=f"header_{sn}",
        ...
        for t in range(TIERS):
            ...
            VisualCuboid(f"/World/Irrig/Tee_{sn}_T{t}", name=f"tee_{sn}_t{t}",
    ...
    # PE perforated duct (center aisle ceiling)
    # Main duct body — rigid PE pipe
    VisualCuboid("/World/HVAC/PEDuct", name="pe_duct",
                 position=np.array([0, 0, duct_z]),
                 scale=np.array([DUCT_DIAM, ROOM_D * 0.85, DUCT_DIAM]),
                 color=PE_DUCT_WHT)
    # Duct end caps
    for ei, ey in enumerate([-ROOM_D*0.425, ROOM_D*0.425]):
        VisualCuboid(f"/World/HVAC/DuctCap_{ei}", name=f"duct_cap_{ei}",
                     position=np.array([0, ey, duct_z]),
    ...
    # Pattern: holes at 45° and 315° from bottom, spacing per Zhang 2016
    ...
    for hi in range(n_holes):
        ...
        # Left-side holes (toward left rack)
        VisualSphere(f"/World/HVAC/Hole_L_{hi}", name=f"hole_l_{hi}",
                     position=np.array([-DUCT_DIAM/2 + 0.01, hy, duct_z - 0.04]),
                     radius=HOLE_DIAM/2, color=np.array([0.15, 0.15, 0.18]))
        # Right-side holes (toward right rack)
        VisualSphere(f"/World/HVAC/Hole_R_{hi}", name=f"hole_r_{hi}",
                     position=np.array([DUCT_DIAM/2 - 0.01, hy, duct_z - 0.04]),
                     radius=HOLE_DIAM/2, color=np.array([0.15, 0.15, 0.18]))
    ...
    for side_idx, rx in enumerate([rack_x_left, rack_x_right]):
        sn = "L" if side_idx == 0 else "R"
        fd = 1 if side_idx == 0 else -1
        for t in range(min(TIERS, 4)):  # Top 4 tiers get CO₂ nozzles
            tz = T_BASE + t * T_GAP + 0.12  # canopy level
            noz_x = rx + fd * 0.05
            VisualCuboid(f"/World/CO2/CropNoz_{sn}_T{t}", name=f"co2_noz_{sn}_t{t}",
    ...
    # Cooling tubes running into each tier gutter (both racks)
    for side_idx, rx in enumerate([rack_x_left, rack_x_right]):
        sn = "L" if side_idx == 0 else "R"
        fd = 1 if side_idx == 0 else -1
        for t in range(TIERS):
            tz = T_BASE + t * T_GAP + 0.025  # Crown level in gutter
            gut_cx = rx + fd * GUTTER_W * 0.5
            # Cooling tube running along gutter length (PE 16mm)
            VisualCuboid(f"/World/Chiller/Tube_{sn}_T{t}", name=f"cool_tube_{sn}_t{t}",
                         position=np.array([gut_cx - fd * 0.04, 0, tz]),
                         scale=np.array([0.016, RACK_L - 0.10, 0.016]),
                         color=COOL_TUBE)
            # T-connector at tier entry
            VisualCuboid(f"/World/Chiller/Tee_{sn}_T{t}", name=f"cool_tee_{sn}_t{t}",
                         position=np.array([gut_cx - fd * 0.04, -RACK_L/2 + 0.05, tz]),
                         scale=np.array([0.025, 0.025, 0.012]),
                         color=COOL_TUBE * 0.9)
            # Crown temperature sensor (per tier)
            VisualSphere(f"/World/Chiller/TempSensor_{sn}_T{t}",
                         name=f"crown_temp_{sn}_t{t}",
                         position=np.array([gut_cx, RACK_L/4, tz + 0.02]),
                         radius=0.006,
                         color=np.array([0.9, 0.3, 0.1]))  # Orange = temp sensor

    flush(6)
    print(f"   ✓ Crown-cooling: chiller + {TIERS*2} tier tubes + {TIERS*2} temp sensors")
    ...
    # ═══════════════════════════════════════════════════════════
    iot_total = TIERS * 2 * 3
    temp_sensors = TIERS * 2
    plant_total = (PPT//2) * TIERS * 2
    co2_nozzles = min(TIERS, 4) * 2
    ...
    # 4. Realistic Robotics Payload: Franka Emika Panda Arm
    ...
    if assets_root is not None and HAS_ASSET_LOADER:
        franka_usd = assets_root + "/Isaac/Robots/Franka/franka.usd"
        franka_prim_path = "/World/GantrySys/BridgeY/CarriageX/ArmZ/Franka"
        add_reference_to_stage(franka_usd, franka_prim_path)
        
        franka_xform = UsdGeom.Xform.Get(stage, franka_prim_path)
        if not franka_xform:
            franka_xform = UsdGeom.Xform.Define(stage, franka_prim_path)
            
        franka_api_static = UsdGeom.XformCommonAPI(franka_xform.GetPrim())
        # Hang upside down from the carriage
        franka_api_static.SetTranslate(Gf.Vec3d(0, 0, 0))
        franka_api_static.SetRotate(Gf.Vec3f(180, 0, 90))
    else:
