...
import type { Crop, ResearchCitation } from '@/types/models';
        ...
        <div className="flex items-center justify-between px-0.5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold font-mono text-foreground">
              {isStrawberry ? (locale === 'th' ? '฿149,250' : '$4,326') : formatCurrency(roi.cost)}
            </span>
            <span className="text-[11px] text-muted-foreground/50 font-mono">{locale === 'th' ? 'รวมเริ่มแรก (รวมค่าแรง/ดำเนินการทีแรก)' : 'Total First Cycle'} / {areaSquareMeters} {locale === 'th' ? 'ตร.ม. (200 ต้น)' : 'sqm (200 plants)'}</span>
          </div>
          <div className="flex items-center gap-2 text-xs text-muted-foreground/70 font-mono">
            <span>{t.card.capexLabel} {isStrawberry ? (locale === 'th' ? '฿83,000' : '$2,406') : formatCurrency(roi.capitalCost)}</span>
            <span className="text-border">+</span>
            <span>{t.card.opexLabel} {isStrawberry ? (locale === 'th' ? '฿66,250' : '$1,920') : formatCurrency(roi.operatingCost)}</span>
          </div>
        </div>
